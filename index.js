            ...
            if (urlMime === "text/html") {
                ...
                data = data.toString().replace(regex, (match, p1, p2) => {
                    ...
                    if (p2.indexOf("http") !== -1) {
                        ...
                        let protoHost =
                            searchURL.protocol + "//" + searchURL.host;
                        newUrl = protoHost + p2;

                        if (lastProtoHost != protoHost) {
                            lastProtoHost = protoHost;
                            console.log(
                                `Using '${protoHost}' as base for new requests.`
                            );
                        }
                    }
                    return ` ${p1}="https://${req.hostname}?url=${newUrl}"`;
                });
            }
            res.type(urlMime);
            data = data
                ...
                .replace(
                    "<head>",
                    '<script type="text/javascript">window.addEventListener("message", (event) => {if(event && event.data==="load")document.getElementsByClassName("ui huge primary button")[0].click()});</script>'
                );
            res.send(data);
        })
        .catch((error) => {
